import { QuestionModel } from "@/app/interface/MainInterfaces";
...
const QuizApi = () => {
  ...
  const handleCreateQuestion = async (
    ...
    try {
      ...
      if (response.ok) {
        const createdQuestion = await response.json(); // Parse the JSON response
  ...
  const handleDeleteQuestion = async (questionId: number) => {
    try {
      const token = localStorage.getItem("jwt");
      const response = await fetch(`${quiz_API}questions/${questionId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
